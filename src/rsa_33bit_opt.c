...
/* Branchless version of the Montgomery multiply. Same result as the
   baseline, but the two ? : selects and the final while loop are gone,
   so nothing branches on the operand values. */
                  ...
                  unsigned int m)
{
    unsigned int i;
    unsigned long T = 0UL;
    unsigned long Y0 = Y & 1UL;
    unsigned long Xi, eta, d, mask;

    for (i = 0; i < m; i++) {
        Xi  = X & 1UL;
        eta = (T & 1UL) ^ (Xi & Y0);

        /* 0 - bit is all zeros or all ones, so "bit ? V : 0" is "V & (0 - bit)" */
        T = (T + (Y & (0UL - Xi)) + (M & (0UL - eta))) >> 1;
        X >>= 1;
    }

    /* T < 2M at this point, so one subtraction is enough. If T < M the
       subtract borrows and we add M back, otherwise we keep the result. */
    d = T - M;
    mask = 0UL - (d >> (sizeof(unsigned long) * 8 - 1));
    return d + (M & mask);
}
